...
class AttendPrintClass implements Printable
{
	Connection con;
	Statement stm;
    ResultSet rs;
	...
	AttendPrintClass(String s,String v)
	{
        userid =s;
        ip=v;
		try
		{
			...
			rs=stm.executeQuery("Select * from Attendance where Empid=(select Account.empid from Account where username='"+userid+"')");
